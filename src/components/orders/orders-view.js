...
export class OrdersView {
    ...
    showOrder(order) {
        ...
        if (order.scheduledDate) {
            document.getElementById('scheduled').innerText = (new Date(order.scheduledDate)).toLocaleDateString('ru');
        }
        if (order.completeDate) {

        }
        document.getElementById('complete').innerText = order.completeDate ? (new Date(order.completeDate)).toLocaleDateString('ru') : '(Заказ не выполнен)';
        if (order.deadlineDate) {
            document.getElementById('deadline').innerText = (new Date(order.deadlineDate)).toLocaleDateString('ru');
        }
        ...
        if (order.createdAt) {
            ...
            document.getElementById('created').innerText = date.toLocaleString('ru');
        }
    }
